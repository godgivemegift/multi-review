...
const PR_STATE: Record<string, { label: string; cls: string }> = {
  open: { label: '进行中', cls: 'text-default border-accented' },
  merged: { label: '已合并', cls: 'text-highlighted border-accented' },
  closed: { label: '已关闭', cls: 'text-dimmed border-default' },
  draft: { label: '草稿', cls: 'text-dimmed border-default' },
}
function prStateBadge(s: string) {
  return PR_STATE[s] ?? { label: '—', cls: 'text-dimmed border-default' }
}
// 任务行 PR 徽章：把 GitHub 评审决定叠进生命周期 —— 进行中 → 已批准 / 请改动 → 已合并 / 已关闭
function prBadge(r: ReviewRow) {
  if (r.prState === 'open' && r.reviewDecision === 'APPROVED') return { label: '已批准', cls: 'text-highlighted border-accented' }
  if (r.prState === 'open' && r.reviewDecision === 'CHANGES_REQUESTED') return { label: '请改动', cls: 'text-toned border-accented' }
  return prStateBadge(r.prState)
}
// 全部 PR 列表的徽章：同样叠进评审决定；草稿/已合并/已关闭照旧
function pullBadge(p: Pull) {
  if (p.state === 'open' && p.reviewDecision === 'APPROVED') return { label: '已批准', cls: 'text-highlighted border-accented' }
  if (p.state === 'open' && p.reviewDecision === 'CHANGES_REQUESTED') return { label: '请改动', cls: 'text-toned border-accented' }
  ...
  <div class="max-w-4xl mx-auto px-10 py-12">
    ...
    <div class="mt-10 flex gap-8 border-b border-default text-sm">
      ...
      <button
        ...
        @click="tab = 'tasks'"
      >
        审核任务 <span class="text-dimmed">{{ tasks?.length || 0 }}</span>
      </button>
      <button
        class="pb-3 -mb-px border-b-2 transition-colors"
      ...
      <div class="mt-4">
        ...
        >
          ...
          <button class="truncate text-default text-left hover:text-highlighted" :title="p.title" @click="openDetail(p.number, p.taskId)">
            {{ p.title }}
            <span v-if="p.hasTask" class="ml-2 text-[10px] text-dimmed">· 已建任务</span>
          </button>
        ...
        <div v-if="pullsResp && (pullsResp.totalCount > PER_PAGE)" class="flex items-center justify-between mt-5 text-xs text-dimmed">
          ...
          <div class="flex gap-4">
            <button class="hover:text-highlighted disabled:opacity-30" :disabled="page === 0 || pullsPending" @click="prevPage">← 上一页</button>
            <button class="hover:text-highlighted disabled:opacity-30" :disabled="!pullsResp.hasNextPage || pullsPending" @click="nextPage">下一页 →</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-show="tab === 'tasks'" class="mt-8">
      ...
      >
        ...
        <span class="text-xs flex flex-col gap-0.5 leading-tight">
          ...
          <span v-if="r.authorUpdated" class="text-[10px] text-highlighted font-medium" title="作者在你上次发评论后又 push 了，可复查">● 作者已更新</span>
        </span>
        ...
        <div class="flex items-center gap-1 justify-end opacity-0 group-hover:opacity-100 transition-opacity">
          <button class="text-dimmed hover:text-highlighted" :class="{ 'opacity-100 animate-spin': refreshing === r.id }" title="刷新 PR 状态" @click="refreshTask(r)">↻</button>
          <button class="text-dimmed hover:text-highlighted" title="删除任务" @click="deleteTask(r)">✕</button>
        </div>
      </div>
      ...
      <div v-if="taskPages > 1" class="flex items-center justify-between mt-5 text-xs text-dimmed">
        ...
        <div class="flex gap-4">
          <button class="hover:text-highlighted disabled:opacity-30" :disabled="taskPage === 0" @click="taskPage--">← 上一页</button>
          <button class="hover:text-highlighted disabled:opacity-30" :disabled="taskPage >= taskPages - 1" @click="taskPage++">下一页 →</button>
        </div>
      </div>
    </div>
