...
// M2 对话跟进：在修复出稿后继续聊、继续改（claude --resume 续会话）。
// ready/error 状态可发；同一 fix 同时只允许一个 chat。后台异步跑，进度走 SSE。
...
export default defineEventHandler(async (event) => {
  ...
  if (!['ready', 'error', 'pushed', 'conflict'].includes(fix.status)) {
    throw createError({ statusCode: 409, statusMessage: `当前状态（${fix.status}）不能对话，请先跑一轮修复` })
  }
