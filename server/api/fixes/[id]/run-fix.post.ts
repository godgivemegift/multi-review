...
export default defineEventHandler(async (event) => {
  ...
  if (!['awaiting', 'ready', 'error'].includes(fix.status)) {
    throw createError({ statusCode: 409, statusMessage: `当前状态（${fix.status}）不能跑修复` })
  }
  ...
  d.update(schema.fixes).set({ status: 'queued', error: null, updatedAt: new Date().toISOString() }).where(eq(schema.fixes.id, id)).run()
